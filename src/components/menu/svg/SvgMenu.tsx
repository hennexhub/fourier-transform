...
export const SvgMenu = ({ setPath }: { setPath: (path: Point[]) => void }) => {
	...
	useEffect(() => {
		...
		Promise.all(fileNames.map(fetchCSV)).then((texts) => {
			const parsedPaths = texts.map((text) => {
				...
				let inputPathData: number[][] = parsed.data
					.map((row) => {
										...
										<Card>
											<CardContent className="flex items-center justify-center p-6 h-full w-full">
												{currentStrokeSettings && currentColorSettings ? (
													<StaticSVGPathRenderer
														colors={currentColorSettings}
														inputPath={path}
														strokes={currentStrokeSettings}
														viewPort={{
															minY: -100,
															minX: -200,
															height: 200,
